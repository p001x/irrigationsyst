...
// Crop profiles (mirrors server.js CROP_DATA)
const CROP_DATA = {
...
const MARKET_PRICE_RWF_T = 350000;
...
function tab(id) {
  ...
  ['inputs','risk','ledger','finance','map','impact','model','export','analysis','faq'].forEach(function(t) {
...
async function updateROI(baseWRSI, optWRSI, dekad) {
    try {
        ...
        if (data.members) {
            GLOBAL_MEMBERS = data.members;
        }

        const delta = data.baseRevenue > 0 ? Math.round(((data.seasonalRevenue - data.baseRevenue) / data.baseRevenue) * 100) : 0;
        const memberAvg = GLOBAL_MEMBERS.length > 0 ? Math.round(data.seasonalRevenue / GLOBAL_MEMBERS.length) : 0;
        ...
        const statusHtml = `
                ...
                <div style="font-size:20px; font-weight:800; color:var(--cyan);">${data.optimalCount} / ${GLOBAL_MEMBERS.length}</div>
...
async function fetchOnlineNDVI() {
    ...
    try {
        ...
        // Get bounding box coordinates for Earth Engine
        ...
        // Fetch Real Data from Earth Engine Backend
        safeSet('online-status', `⏳ Fetching real data from Google Earth Engine for ${dateVal}...`);
        
        const response = await fetch(`/api/spectral-data?date=${dateVal}&bounds=${JSON.stringify(coords)}`);
        
        if (!response.ok) {
            const errData = await response.json();
            throw new Error('GEE Backend Error: ' + errData.error);
        }
        
        const geeData = await response.json();
        
        const ndvi = geeData.ndvi;
        const ndwi = geeData.ndwi;
        const spi  = geeData.spi;
        ...
        const sourceLabel = layer.includes('Landsat') ? 'Earth Engine Landsat 8/9' : 'Earth Engine MODIS Terra + CHIRPS';
        displaySpectralResults(ndvi, ndwi, spi, `${sourceLabel} (Real Data) · ${dateVal}`);
        safeSet('online-status', `✅ Real GEE data loaded for ${dateVal} (Spatial Extent Locked)`);
    } catch (err) {
    ...
    const html = `
        ...
        <div class="inner-glass" style="padding:20px;">
            ...
            <div style="margin-top:15px; padding-top:12px; border-top:1px solid var(--border); font-size:10px; color:var(--ink-mute); font-family:var(--mono);">
                Source: ${source === 'sim' ? 'Simulated from dashboard sliders' : source === 'band' ? 'Computed from uploaded GeoTIFF bands (pixel-level mean)' : (source.includes('Real Data') ? source : 'NASA GIBS WMS + simulation state')} · ${new Date().toLocaleString()}
            </div>
        </div>
    `;
